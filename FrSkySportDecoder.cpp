/*
  FrSky telemetry decoder class for Teensy 3.x/4.0/LC, ESP8266 and ATmega328P based boards (e.g. Pro Mini, Nano, Uno)
  (c) Pawelsky 20191110
...
uint16_t FrSkySportDecoder::decode()
{
  ...
  if(serial.port != NULL)
  {
    ...
    if(serial.port->available())
    {
      ...
      else if((byte == FRSKY_STUFFING) && (state > DATA_FRAME) && (state <= CRC)) hasStuffing = true; // Skip stuffing byte in data and mark to xor next byte with 0x20
      else
      {
